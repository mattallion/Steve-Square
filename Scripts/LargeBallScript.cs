...
public class LargeBallScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (steve.score == 3)
        {
            ...
            gameObject.GetComponent<LargeBallScript>().enabled = false; //change the green portion between <> to current script name
        }
                                              ...
                                              // you will have to customize it to act according to the power-up or power-down
    {
        if (coll.gameObject.tag == "Player")
        {
            ...
            bouncers[0].largeBall();
            bouncers[1].largeBall();
            bouncers[2].largeBall();
            Destroy(gameObject);
        }
    }
}
