...
public class OneLessBall : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (steve.score == 3)
        {
            ...
            gameObject.GetComponent<OneLessBall>().enabled = false; //change the green portion between <> to current script name
        }
        ...
        {
            gameObject.GetComponent<SpriteRenderer>().enabled = false;
            gameObject.GetComponent<OneLessBall>().enabled = false; //change the GREEN words between <> to current script name
        }
    }
